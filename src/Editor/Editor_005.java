...
public class Editor_005 extends JFrame {
			...
			for (int j = x; j < x+centerPanelSizeX/64+1+scrOffsetX; j++) {
				...
				if ((i == (yCor+scrOffsetY+centerPanelSizeY/64/2)) && (j == (xCor+scrOffsetX+centerPanelSizeX/64/2))) {
					...
					for (int k = 0; k < numbOfObjects[scrOffsetY+centerPanelSizeY/64/2][scrOffsetX+centerPanelSizeX/64/2]; k++) {
						...
						infoText += "dXY= " + dXY + "\n";
						infoText += "Y/X= " + dY + "/"+ dX +"\n";
						infoText += "Высота= "+Integer.toString(TileArray[i][j][k*8+20]) + "\n";
						infoText += "Порядок= "+Integer.toString(TileArray[i][j][k*8+17]/16 + TileArray[i][j][k*8+18]*16 + TileArray[i][j][k*8+19]*4096) + "\n";
	...
	public Editor_005()		//Конструктор класса
	{
	...
	private JMenu createViewMenu()
    {
        ...
        JCheckBoxMenuItem type  = new JCheckBoxMenuItem("Тип плитки");
        JCheckBoxMenuItem numObj = new JCheckBoxMenuItem("Показать объекты");
        JCheckBoxMenuItem form = new JCheckBoxMenuItem("Контур объектов на плитке");
        JCheckBoxMenuItem formAll = new JCheckBoxMenuItem("Контур объектов на всех плитках");
        JCheckBoxMenuItem Height = new JCheckBoxMenuItem("Высоты");
	...
	public static void main(String[] args) throws IOException {
		...
		ObjectsName = ReadObjectsInfo.readObjectsName(inpObjFile);
		Editor_005 mainFrame = new Editor_005();
